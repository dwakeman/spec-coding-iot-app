...
**Symptom (in code/queries):** Presto queries like `SELECT * FROM cassandra_catalog.workshop.customers` fail with `Schema 'cassandra_catalog.workshop' does not exist` or similar.

**Diagnostic:** ask the attendee to open `https://localhost:9443` and check **Infrastructure Manager → Databases**. If `cassandra_workshop` isn't listed, registration was never done. If it's listed but the catalog name doesn't match `cassandra_catalog`, the form was filled wrong.

**Action — direct the attendee to register Cassandra now:**
...
>
> Click **Test connection** (it should succeed), then **Add**. The catalog is created in the same form — no separate associate step."

After they confirm, retry the failing query. If it still fails, the catalog name might be wrong — check **Databases** in the UI, click the entry, and verify the catalog reads `cassandra_catalog` exactly.
